...
def ensure_client(wait=0):
    ...
    if client is None:
        ...
        source = BridgeClient(values["pipe"], wait_for=retiring_worker, quality=values["quality"])
        source.full_xp_volume = values["fullXPVolume"]
        source.callback = lambda event, data: wx.CallAfter(_deliver, source, event, data)
        client = source
        try:
            source.start()
        except Exception:
