...
| Announce bridge connection changes through NVDA | On by default. When the bridge runs in the background behind a local synthesizer, NVDA speaks each connection and disconnection once. Uncheck it to keep the status available only in settings and the log. |
